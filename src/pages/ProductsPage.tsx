    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="sticky top-0 z-40 bg-background/95 backdrop-blur-md px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <Sheet>
            ...
            <SheetContent className="w-80">
              <SheetHeader>
                <SheetTitle className="font-display text-xl">Bộ lọc</SheetTitle>
              </SheetHeader>
              <div className="mt-6 space-y-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {allColors.map(c => (
                      <button key={c} onClick={() => toggleColor(c)}
                        className={`rounded-none border px-3 py-1.5 text-xs font-body transition-colors ${selectedColors.includes(c) ? 'border-foreground bg-foreground text-background' : 'border-border'}`}>
                        {c}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-2">
                    {allSizes.map(s => (
                      <button key={s} onClick={() => toggleSize(s)}
                        className={`flex h-10 w-10 items-center justify-center border text-xs font-body font-medium transition-colors ${selectedSizes.includes(s) ? 'border-foreground bg-foreground text-background' : 'border-border'}`}>
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                {hasFilters && (
                  <button onClick={clearFilters} className="flex items-center gap-1 text-xs text-muted-foreground underline font-body">
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto no-scrollbar">
          <button onClick={() => setCategory('')}
            className={`shrink-0 rounded-none border px-4 py-1.5 text-xs font-body font-medium transition-colors ${!category ? 'border-foreground bg-foreground text-background' : 'border-border'}`}>
            Tất cả
          </button>
          {categories.map(cat => (
            <button key={cat.id} onClick={() => setCategory(cat.id)}
              className={`shrink-0 rounded-none border px-4 py-1.5 text-xs font-body font-medium transition-colors ${category === cat.id ? 'border-foreground bg-foreground text-background' : 'border-border'}`}>
              {cat.icon} {cat.name}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 && (
        <div className="py-20 text-center">
          <p className="text-muted-foreground font-body text-sm">Không tìm thấy sản phẩm phù hợp</p>
        </div>
      )}
    </div>
  );
