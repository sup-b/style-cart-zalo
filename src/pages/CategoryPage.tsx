...
import { Shirt, Briefcase, Sparkles, Watch, ShoppingBag, Tag, Gem, Heart, ArrowLeft } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-background pb-20">
        <div className="sticky top-0 z-40 flex items-center gap-3 border-b border-border bg-background/95 backdrop-blur-md px-4 py-3">
          <button onClick={() => setSelectedCategory(null)} className="flex h-8 w-8 items-center justify-center rounded-full bg-secondary">
          ...
          <span className="ml-auto font-body text-xs text-muted-foreground">{filteredProducts.length} sản phẩm</span>
        </div>
        {isLoading ? (
          ...
          <>
            ...
            {filteredProducts.length === 0 && (
              <div className="py-20 text-center"><p className="font-body text-sm text-muted-foreground">Chưa có sản phẩm trong danh mục này</p></div>
            )}
          </>
        )}
      </div>
      ...
      <div className="px-4 py-5">
        <div className="grid grid-cols-4 gap-y-4">
          {categoryLinks.map(({ id, name, icon: Icon, categoryId }) => (
            <button key={id} onClick={() => categoryId ? setSelectedCategory(id) : undefined}
              className="flex flex-col items-center gap-1.5 transition-transform active:scale-95">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-secondary">
                <Icon className="h-5 w-5 text-foreground/70" strokeWidth={1.5} />
              </div>
              <span className="w-full truncate text-center font-body text-xs text-muted-foreground">{name}</span>
            </button>
          ))}
        </div>
      </div>
